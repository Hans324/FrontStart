...
<section>
<h3 className='Intro-title-two'>Créer ton premier document HTML</h3>
<p className='paragraphe-three'>Voyons comment les éléments individuels sont combinés pour former une page HTML. Dans cette section, 
    vous allez créer un fichier HTML de base et examiner sa composition.
</p>
<p className='paragraphe-four'>
...
<p className='paragraphe-five'>Voici ce que chaque ligne de code dit:</p>

<p className='paragraphe-six'>!doctype html : Le docytype est un préambule obligatoire. Aux débuts de HTML( vers 1991/92), les doctypes
...
<p className='paragraphe-seven'> L'élément "html" englobe tout le contenu de la page et est parfois appelé élément racine. Il inclut aussi l'attribut lang, qui définit la langue principale du document.</p>
<p className='paragraphe-eight'> L'élément head sert de conteneur pour tout ce que vous souhaitez inclure dans la page HTML qui n'est pas du contenu affiché aux visiteur·euse·s. Cela inclut par exemple des mots-clés et une description de la page pour les résultats de recherche, du CSS pour la mise en forme, des déclarations d'encodage de caractères, etc.</p>

<p className='paragraphe-nine'> meta charset="utf-8" : Cet élément définit l'encodage de caractères que votre document doit utiliser, à savoir UTF-8, qui inclut la plupart des caractères de la grande majorité des langues écrites.
...
<p className='paragraphe-ten'> meta name="viewport" content="width=device-width" : Cet élément viewport garantit que la page s'affiche à la largeur de la zone d'affichage du navigateur, empêchant les navigateurs mobiles d'afficher des pages plus larges que la zone d'affichage puis de les réduire.
    </p>
<p className='paragraphe-eleven'>title: L'élément title définit le titre de votre page, qui apparaît dans l'onglet du navigateur où la page est chargée. Il sert aussi à décrire la page lorsqu'on l'ajoute aux favoris.</p>
<p className='paragraphe-twelve'> body : L'élément body contient tout le contenu que vous souhaitez afficher aux utilisateur·ice·s web lorsqu'ils visitent votre page, qu'il s'agisse de texte, d'images, de vidéos, de jeux, de pistes audio, etc. Pour l'instant, il ne contient qu'un seul élément img, mais nous ajouterons plus de contenu par la suite.</p>

<p className='paragraphe-thirteen'>Note : La plupart des éléments HTML se composent d'une balise ouvrante par exemple, body, suivie du contenu de l'élément, puis d'une balise fermante par exemple, body. Certains éléments HTML possèdent aussi des attributs, qui contiennent des réglages ou des informations supplémentaires sur l'élément — voyez par exemple charset, name et src dans notre exemple de code.</p>
</section>
